...
class ProfileImporter {
	...
	public function computeMultisiteToolsDetails( $profile ) {
		...
		if (
			...
			// Ideally we'd cast selected_subsite and destination_subsite to int,
			// however this formatting needs to happen very early, before the CLI
			// has enough information to convert remote subsite URLs to subsite ID.
			$output['selected_subsite'] = $profile->mst_selected_subsite;

			if ( isset( $profile->mst_destination_subsite ) ) {
				$output['destination_subsite'] = $profile->mst_destination_subsite;
			}
	...
	public function computeAdvancedOptions( $profileObj, $intent ) {
		...
		if ( ! isset( $allowed_advanced_options[ $intent ] ) ) {
