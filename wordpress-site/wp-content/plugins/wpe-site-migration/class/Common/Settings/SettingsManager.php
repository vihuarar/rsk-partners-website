...
class SettingsManager {
	...
	public function ajax_whitelist_plugins() {
		...
		$result = $this->settings->set_setting( 'whitelist_plugins', (array) $state_data['whitelist_plugins'] );

		if ( false === $result ) {
			$this->http->end_ajax(
				new \WP_Error(
					'wpmdb-whitelist_plugins-failed',
					__( 'Failed to save migration options. Please try again.', 'wp-migrate-db' )
				)
			);
