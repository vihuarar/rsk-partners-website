...
class Replace {
	...
	/**
	 * Getter for given property name.
	 *
	 * @param string $prop Property name to get value for.
	 *
	 * @return mixed
	 */
	...
	/**
	 * Setter for given property name.
	 *
	 * @param string $prop  Property name to update.
	 * @param mixed  $value Value to update property with.
	 *
	 * @return mixed
	 */
	...
	/**
	 * Register options for search/replace.
	 *
	 * @param array $args Associative array of search/replace options.
	 *
	 * @return $this
	 */
	public function register( $args ) {
		...
		$this->get_diff_interpreter()->getGroup()->setTable( $this->table );
	...
	/**
	 * Retrieve search/replace pairs from stored migration options.
	 *
	 * @param string $intent   Optional migration intent.
	...
	public static function parse_find_replace_pairs( $intent = '', $site_url = '' ) {
		...
		if (
			...
			foreach ( $standard_pairs as $key => $pair ) {
				if (
					! empty( trim( $pair['replace'] ) ) &&
					in_array( $key, $migration_options['search_replace']['standard_options_enabled'], true )
				) {
	 ...
	 * @return bool
	 */
	private function is_subdomain_replaces_on() {
	 ...
	 * @return bool
	 */
	private function has_same_base_domain() {
	 ...
	 * @return mixed
	 */
	public function subdomain_replaces( $new ) {
	 ...
	 * @return bool
	 */
	private function detect_protocol_mismatch() {
	 ...
	 * @return mixed
	 */
	public function do_protocol_replace( $new, $destination_url ) {
		/**
	...
	/**
	 * Add search/replace pairs for JSON handling versions.
	 *
	 * @return bool
	 */
	...
	public function apply_replaces( $subject ) {
		...
		if ( 'find_replace' === $this->intent ) {
			...
			$this->get_diff_interpreter()->compute(
				DiffEntity::create(
					$original,
					$subject,
	...
	public function recursive_unserialize_replace(
		...
		if (
			$this->should_do_reference_check( $table_prefix ) &&
			is_serialized( $data ) &&
			preg_match( '/r\:\d+;/i', $data )
		) {
	 ...
	 *
	 * @return object The row currently being processed in the migration.
	 */
	 ...
	 *
	 * @param object $row Row currently being processed in the migration.
	 */
	...
	/**
	 * Multisite safe way of comparing the table currently being processed in the migration against a desired table.
	 *
	...
	public function get_diff_result() {
		$result = [];
		foreach ( $this->get_diff_interpreter()->getGroup()->getEntities() as $entity ) {
