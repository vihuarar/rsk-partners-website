...
class Util {
	...
	/**
	 * Get human readable version of a DateInterval.
	 *
	 ...
	 *
	 * @param DateInterval|mixed $interval Interval to be converted.
	 *
	 * @return string
	 */
	public static function hr_interval( $interval ) {
		$units = [
			'y' => _x( 'Y', 'Years indicator for display of interval.', 'wp-migrate-db' ),
			'm' => _x( 'M', 'Months indicator for display of interval.', 'wp-migrate-db' ),
			'd' => _x( 'D', 'Days indicator for display of interval.', 'wp-migrate-db' ),
			'h' => _x( 'h', 'Hours indicator for display of interval.', 'wp-migrate-db' ),
			'i' => _x( 'm', 'Minutes indicator for display of interval.', 'wp-migrate-db' ),
			's' => _x( 's', 'Seconds indicator for display of interval.', 'wp-migrate-db' ),
		];

		if ( empty( $interval ) || ! is_a( $interval, '\DateInterval' ) ) {
			return '0' . $units['s'];
		}
