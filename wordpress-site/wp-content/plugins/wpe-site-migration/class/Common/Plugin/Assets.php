...
class  Assets {
		...
		$profiles = array_map(
			function ( $key, $item ) {
				return [
					...
					'guid'           => $item['guid'],
					'name'           => $item['name'],
					'intent'         => $this->get_intent_from_profile( $item ),
	...
	/**
	 * Gets the intent from the migration profile data.
	 *
	 * @param array $item
	 * @return string|null Intent value or null if unavailable.
	 */
	private function get_intent_from_profile( $item ) {
		...
		if ( ! isset( $value->current_migration->intent ) ) {
			return null;
		}

		return $value->current_migration->intent;
	}
