...
class InitiateMigration {
	...
	public function initiate_migration( $state_data = false ) {
		...
		if ( empty( $state_data['intent'] ) ) {
			return new WP_Error(
				'wpmdb-initiate-migration-no-intent',
				__( 'No intent given.', 'wp-migrate-db' )
			);
		}

		if ( empty( $state_data['form_data'] ) ) {
			return new WP_Error(
				'wpmdb-initiate-migration-no-form-data',
				__( 'No form data given.', 'wp-migrate-db' )
			);
		}
		...
		if ( in_array( $state_data['intent'], array( 'push', 'pull' ) ) ) {
			$return = $this->initiate_push_or_pull( $state_data );
		} else {
			$return = $this->initiate_local_migration( $state_data );
		}
	 ...
	 */
	protected function initiate_push_or_pull( array $state_data ) {
		if ( ! isset( $state_data['intent'] ) || ! in_array( $state_data['intent'], array( 'push', 'pull' ) ) ) {
			return new WP_Error(
				'wpmdb-initiate-push-or-pull-invalid-intent',
				__( 'Invalid intent given.', 'wp-migrate-db' )
			);
		}
	 ...
	 * @return array|WP_Error
	 */
	protected function initiate_local_migration( $state_data ) {
		if ( empty( $state_data['intent'] ) ) {
			return new WP_Error(
				'wpmdb-initiate-local-migration-no-intent',
				__( 'No intent given.', 'wp-migrate-db' )
			);
		}
		...
		if ( Stage::BACKUP === $state_data['stage'] || ( Stage::MIGRATE && 'import' !== $state_data['intent'] ) ) {
